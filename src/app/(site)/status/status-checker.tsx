...
const STATUS_EXPLANATION: Record<string, string> = {
  SUBMITTED: "We have your application and payment proof. Our team is verifying it — usually within 2 working days.",
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        />
        <Button type="submit" loading={pending} disabled={query.trim().length < 4}>
          Check status
        </Button>
      </form>
      ...
      {entries?.map((entry) => (
        <StatusCard key={entry.reference} entry={entry} onChanged={() => lookup(query)} />
      ))}
    </div>
  );
}

function StatusCard({ entry, onChanged }: { entry: StatusEntry; onChanged: () => void }) {
  ...
  return (
    <article className="rule-gold rounded-b-brand border border-line bg-white/70 p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="font-display text-step-1">{entry.playerFirstName}&apos;s application</p>
        <StatusBadge status={entry.status} />
      </div>
      <p className="mt-1 font-mono text-step--1 text-kit-soft">
        {entry.reference} · submitted {formatDateWAT(new Date(entry.submittedAtIso))}
      </p>
      ...
      {entry.rejectionReason && (
        <p className="mt-2 text-step--1 text-kit-soft">
          Reason: {entry.rejectionReason}. You can submit a fresh application from the Enroll page,
          or call us if you believe this was a mistake.
        </p>
      )}

      {entry.memberCode && (
        <div className="mt-4 flex flex-wrap items-center gap-3 rounded-brand bg-pitch-deep px-4 py-3">
          <p className="font-mono text-step-1 font-bold tracking-widest text-gold">{entry.memberCode}</p>
          <CopyButton value={entry.memberCode} label="member code" className="border-pitch-mid text-chalk" />
        </div>
      )}
      ...
      {entry.canReupload && (
        <div className="mt-5 flex flex-col gap-3 border-t border-line pt-4">
          <p className="text-step--1 font-semibold">
            We asked for a clearer payment proof. Upload it here — your reference stays the same.
          </p>
