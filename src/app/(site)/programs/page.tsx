...
export default async function ProgramsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-[var(--gutter)] py-12">
      ...
      <h1 className="mt-2 font-display text-step-3">
        One pathway, three phases
      </h1>
      <p className="mt-3 max-w-2xl text-step-0 text-kit-soft">
        Every Moyours player boys and girls aged 4–18 moves through a structured
        pathway: from falling in love with the ball to competing in front of
        scouts.
      </p>
      ...
      <div className="reveal mt-10 grid gap-5 md:grid-cols-3">
        {PROGRAM_PHASES.map((phase, index) => (
          ...
          >
            ...
            <h2
              id={`phase-${phase.key}`}
              className="mt-2 font-display text-step-1"
            >
              {phase.name}
            </h2>
            ...
            <ul className="mt-4 flex flex-col gap-2">
              {PHASE_DETAIL[phase.key].map((item) => (
                <li
                  key={item}
                  className="flex gap-2.5 text-step--1 leading-relaxed"
                >
                  <span
                    aria-hidden
      ...
      <section aria-labelledby="squads-h" className="reveal mt-12">
        ...
        <div className="mt-5 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {AGE_GROUPS.map((group) => (
            ...
            >
              <p className="font-mono text-step-2 font-bold text-pitch">
                {group.key}
              </p>
      ...
      <div className="rule-gold mt-12 rounded-b-brand bg-pitch p-5 text-chalk sm:p-6">
        ...
        <p className="mt-3 text-step--1 text-chalk-dim">
          {site.address} · plus a second training location to meet growing
          demand.
        </p>
      </div>
            ...
            <tbody>
              <tr className="border-b border-line">
                ...
                <td className="py-3 text-right font-mono">
                  {formatNaira(fees.registrationKobo)}
                </td>
              </tr>
              <tr className="border-b border-line">
                <th scope="row" className="py-3 text-left font-normal">
              ...
              <tr>
                <th
                  scope="row"
                  className="py-3 text-left font-normal text-kit-soft"
                >
                  Monthly subscription
                </th>
                <td className="py-3 text-right font-mono text-kit-soft">
                  {formatNaira(fees.monthlyKobo)}
                </td>
              </tr>
            </tbody>
          </table>
