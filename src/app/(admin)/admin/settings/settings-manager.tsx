...
export function SettingsManager({
  ...
  const [contact, setContact] = useState({
    ...initialContact,
    phonesText: initialContact.phones.join("\n"),
  });
  ...
  return (
    <div className="flex flex-col gap-5">
          ...
          <Field label="Registration fee (₦)">
            {(a11y) => (
              <Input
                ...
                onChange={(e) =>
                  setFees({ ...fees, registration: e.target.value })
                }
              />
            )}
          </Field>
          <Field label="Jersey, 2 sets (₦)">
            {(a11y) => (
              <Input
                ...
                min={0}
                value={fees.jersey}
                onChange={(e) => setFees({ ...fees, jersey: e.target.value })}
              />
            )}
          </Field>
      ...
      >
        {schedule.map((session, index) => (
          ...
          >
            ...
            <Button
              variant="ghost"
              size="sm"
              aria-label={`Remove ${session.day} session`}
              onClick={() =>
        ...
        <Button
          ...
          onClick={() =>
            setSchedule([
              ...schedule,
              { day: "Sunday", start: "9:00 AM", end: "11:00 AM" },
            ])
          }
        >
          Add session
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Phone numbers" hint="One per line">
            {(a11y) => (
              <Textarea
                ...
                onChange={(e) =>
                  setContact({ ...contact, phonesText: e.target.value })
                }
              />
            )}
          </Field>
          <div className="flex flex-col gap-4">
            ...
            <Field
              ...
              hint="Included in acceptance emails when set"
            >
              {(a11y) => (
                <Input
                  {...a11y}
                  type="url"
                  value={contact.whatsappGroupUrl}
                  onChange={(e) =>
                    setContact({ ...contact, whatsappGroupUrl: e.target.value })
                  }
                />
              )}
            </Field>
      ...
      <Section
        ...
        pending={pending}
        onSave={() =>
          run(() => saveOverdueDay(Number(overdueDay)), "Overdue day saved.")
        }
      >
        <Field
          label="Flag unpaid subscriptions overdue after day"
        ...
        >
          {(a11y) => (
            <Input
              ...
              className="w-28"
              value={overdueDay}
              onChange={(e) => setOverdueDay(e.target.value)}
            />
          )}
        </Field>
      </Section>
