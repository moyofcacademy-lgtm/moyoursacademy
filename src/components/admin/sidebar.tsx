...
export function AdminSidebar({ adminName }: { adminName: string }) {
      ...
      {adminNav.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "rounded-brand px-3 py-2 text-step--1 text-chalk-dim transition-colors hover:bg-pitch-mid hover:text-chalk",
              active && "bg-pitch-mid font-semibold text-chalk",
            )}
          >
            {item.label}
          </Link>
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-56 flex-col border-r border-pitch-mid bg-pitch-deep text-chalk lg:flex">
        ...
        <div className="border-t border-pitch-mid px-5 py-3">
